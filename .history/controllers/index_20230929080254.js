...
function displayCharacters(chars, n, starts, ends) {
  ...
  words.forEach((word) => {
    ...
    if (n == null && starts == null && ends == null) {
      if (broken === false) {
        [numOfIndexes, seenIndexes] = checkIndexes(
          word,
          numOfIndexes,
          seenIndexes
        );
      ...
      if (broken === false && word.length === n) {
        checkIndexes(word, numOfIndexes, seenIndexes);
        result += word + ",";
      }
    } else if (n != null && starts != null && ends == null) {
...
function checkIndexes(input, numOfIndexes, seenIndexes) {
  let length = input.length;
  if (seenIndexes[length] == 0) {
    seenIndexes[length] = 1;
    numOfIndexes = numOfIndexes + 1;
  }
  return [numOfIndexes, seenIndexes];
}
